         ...
         var initTable = function() {
             oTable = $('#editable').dataTable({
                 ...
                 "columns": [{
                     ...
                     "data": function(e) {
                         if (e.status == 1) {
                             return '未发布' + '<button type="button" class="btn btn-primary btn-xs publish" title="发布" data-toggle="tooltip" ><i class="fa fa-check-square-o"></i></button><button type="button" class="btn btn-danger btn-xs to-black" title="加入黑名单" data-toggle="tooltip" ><i class="fa fa-frown-o"></i></button>';
                         } else if (e.status == 2) {
                             return '已发布' + '<button type="button" class="btn btn-warning btn-xs cel-publish" title="取消发布" data-toggle="tooltip" ><i class="fa fa-close"></i></button><button type="button" class="btn btn-danger btn-xs to-black" title="加入黑名单" data-toggle="tooltip" ><i class="fa fa-frown-o"></i></button>';
                         } else {
                             return '黑名单' + '<button type="button" class="btn btn-default btn-xs cel-black" title="取消黑名单" data-toggle="tooltip" ><i class="fa fa-smile-o"></i></button>';
                         }

                     }
                 }, {
                     "data": function(e) {
                         return '<a class="btn btn-primary btn-xs edit" title="编辑" data-toggle="tooltip" href="/admin/shop/edit?action=edit&id=' + e.id + '"><i class="fa fa-pencil"></i></a>';

                         return '<a class="btn btn-primary btn-xs edit" title="编辑" data-toggle="tooltip" href="/admin/shop/edit?action=edit&id=' + e.id + '"><i class="fa fa-pencil"></i></a>' + '<button type="button" class="btn btn-danger btn-xs delete"  title="删除" data-toggle="tooltip"><i class="fa fa-trash-o "></i></button>';
             ...
             initEvents: function() {
                 ...
                 $('#editable button.publish,#editable button.cel-publish').live('click', function(e) {
                     ...
                     if ($(this).hasClass('publish')) {
                         ...
                         if (!data.title || !data.score || !data.url) {
                     ...
                     var publish = new Remoter('/admin/shopapi/save');
                     publish.remote({
                         id: data.id,
                         action: action
                     });
...
*/
                     ...
                     function changeWeight(id, from, to, sight_id, fromIndex, toIndex) {
                         $.ajax({
                             "url": "/admin/shopapi/changeWeight",
                             "data": {
                                 id: id,
                                 to: to,
                                 sightId: sight_id
                 ...
                 $('#editable button.all-action').live('click', function(e) {
                     ...
                     var publish = new Remoter('/admin/shopapi/changeStatus');
                     publish.remote({
                         idArray: idArray,
                         action: action
                     });
             ...
             editPic: function() {
                 ...
                 $("#ModalUpload-btn").click(function(event) {
                     var upload = new Remoter('/admin/shopapi/save');
                     upload.remote({
                         id: $('#image-id').val(),
                         image: $('#image').val()
                     });
