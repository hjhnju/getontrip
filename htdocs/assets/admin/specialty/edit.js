        ...
        var bindEvents = {
            ...
            init_image: function() {
                ...
                $("#upload-img").click(function(event) {
                    $.ajaxFileUpload({
                        ...
                        success: function(res, status) { //当文件上传成功后，需要向数据库中插入数据
                            ...
                            localStorage.specialtyimage = res.data.image;
                        },
                ...
                $(window).bind('beforeunload', function() {
                    if (localStorage.specialtyimage) {
            ...
            init_sight: function() {
                ...
                $('#city_alert span button').each(function() {
                    city_id_array.push(Number($(this).attr('data-id')));
                });
                //景点输入框自动完成
              ...
              //景点输入框自动完成
                $('#city_name').typeahead({
                    ...
                    itemSelected: function(item, val, text) {
                        ...
                        //先判断是否已经选择
                        var valnum = Number(val);
                        if (!city_id_array.in_array(valnum)) {
                            city_id_array.push(valnum);
                            //添加到右侧选框中 
                            $('#city_alert').append('<span class="badge badge-sm label-danger" role="badge">' + text + '<button type="button" class="close" data-id="' + val + '"><span class="fa fa-remove"></span></button></span>');
                        }
                    }
                });

                //城市框删除城市
                $('#city_alert').delegate('.close', 'click', function(event) {
                    var val = Number($(this).attr('data-id'));
                    city_id_array.splice($.inArray(val, city_id_array), 1);
                    $(this).parent().remove();
                });
                    ...
                    $.ajax({
                        ...
                        "success": function(response) {
                            ...
                            if (response.status == 0) {
                                localStorage.specialtyimage = '';
                                toastr.success('保存成功');
                                  if (url.indexOf('add') >= 0) {
                                    //resetForm();
                                    window.location.href = '/admin/specialty/edit?action=edit&id=' + response.data;
                                } else {
