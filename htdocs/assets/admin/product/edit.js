                ...
                $("#upload-img").click(function(event) {
                    $.ajaxFileUpload({
                        ...
                        success: function(res, status) { //当文件上传成功后，需要向数据库中插入数据
                            ...
                            localStorage.productimage = res.data.image;
                        },
        ...
        var validations = function() {
            $.validator.setDefaults({
                submitHandler: function(data) {
                    ...
                    var url = '';
                    if (!$('#id').val()) {
                        url = '/admin/productapi/add';
                    } else {
                        url = "/admin/productapi/save"
                    }
                    //按钮disabled
                            ...
                            if (response.status == 0) {
                                ...
                                toastr.success('保存成功');
                                  if (url.indexOf('add') >= 0) {
                                    //resetForm();
                                    window.location.href = '/admin/product/edit?action=edit&id=' + response.data;
                                } else {
