 ...
 * author:fanyy
 */
class SpecialtyapiController extends Base_Controller_Api{
    ...
    public function listAction(){
         ...
         $List =Specialty_Api::getSpecialtys($page,$pageSize,$arrParam);
        
         $tmpList = $List['list']; 
         for($i=0; $i<count($tmpList); $i++) {   
            //处理状态
            $tmpList[$i]['statusName'] = Specialty_Type_Status::getTypeName($tmpList[$i]["status"]);
    ...
    function addAction(){
        ...
        $dbRet =Specialty_Api::addSpecialty($_REQUEST);
        if (!empty($dbRet)) {
    ...
    function saveAction(){
        ...
        $dbRet = Specialty_Api::editSpecialty($id, $_REQUEST);
    ...
    function delAction(){
        ...
        $dbRet = Specialty_Api::delSpecialty($id);
        if ($dbRet) {
            return $this->ajax();
        }
        return $this->ajaxError();
    ...
    */
    public function changeWeightAction()
    {
        ...
        $dbRet =  Specialty_Api::changeWeight($destId, $type,$id, $to);
    ...
    public function changeStatusAction(){
       ...
       for ($i=0; $i < count($idArray); $i++) { 
           ...
           $bRet = Specialty_Api::editSpecialty($postid,array('status'=>$status)); 
           if(!$bRet){  
    ...
    public function cropPicAction(){
        ...
        if($ret){
          if(!empty($postid)){
            ...
            //修改话题的图片hash
            $bRet=Specialty_Api::editSpecialty($postid,$params);
            if($bRet){
   ...
   /**
    ...
    public function getStatusByActionStr($action){
        switch ($action) {
         case 'NOTPUBLISHED':
           $status = Specialty_Type_Status::NOTPUBLISHED;
           break;
         case 'PUBLISHED':
           $status = Specialty_Type_Status::PUBLISHED;
           break;
        case 'BLACKLIST':
           $status = Specialty_Type_Status::BLACKLIST;
           break;
         default:
           $status = Specialty_Type_Status::NOTPUBLISHED;
           break;
